...
export const DIFFICULTIES = Object.freeze(['Casual', 'Professional', 'Master']);
...
export const CUSTOM_CHALLENGES = Object.freeze([
	{ name: 'All Targets at Once' },
	{ name: 'No Look Kill (Facing Away from Target)' },
	{ name: 'Must Kill from Cover' },
	{ name: 'Cannot Kill from Cover' },
	{ name: 'Must Include Food In Kill' },
	{ name: 'Cannot Include Food In Kill' },
	{ name: 'Without Entering the Hallways' }
]);
...
export function randomCustomChallenge() {
	return _sample(CUSTOM_CHALLENGES)[0];
}
...
export function customScenario(
	...
	const randomZeroOrOne = () => Math.floor(generator.random() * 2);
	const withOrWithout = ['with', 'without'];
	const mustOrCannot = ['must', 'cannot'];

	let prompt = `You must kill ${map.targets[Math.floor(generator.random() * map.targets.length)]}`;
	if (weapons) {
		const weapon = WEAPON_CATEGORIES[Math.floor(generator.random() * WEAPON_CATEGORIES.length)];
		prompt += `, ${withOrWithout[randomZeroOrOne()]} ${weapon}`;
	}

	if (time) {
		const minutes = Math.floor(generator.random() * 31);
		const seconds = (Math.floor(generator.random() * 120) * 15) % 60;
		prompt += `, in ${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
	}

	if (kills) {
		const challenge = CHALLENGES[Math.floor(generator.random() * CHALLENGES.length)];
		prompt += `, and you ${mustOrCannot[randomZeroOrOne()]} complete the '${challenge.name}' challenge`;
	}

	if (custom) {
		const customChallenge =
			CUSTOM_CHALLENGES[Math.floor(generator.random() * CUSTOM_CHALLENGES.length)];
		prompt += `, ${customChallenge.name}`;
	}
